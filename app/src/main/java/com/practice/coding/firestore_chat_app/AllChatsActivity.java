...
public class AllChatsActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        arrayList.clear();
        FirestoreKeys.chatReference.orderBy(Keys.TIME_STAMP, Query.Direction.DESCENDING).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                        ...
                        if (userId.equals(chatModel.getUserId())) {
                            if (arrayList.isEmpty()) {
                                ...
                                for (int i = 0; i < arrayList.size(); i++) {
                                    if (!arrayListCompareReceiverIds.contains(chatModel.getReceiverId())) {
                                        arrayList.add(chatModel);
                                        arrayListCompareReceiverIds.add(chatModel.getReceiverId());
                                    }
                                }
                    ...
                    Toast.makeText(AllChatsActivity.this, "Error while loading data!", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
